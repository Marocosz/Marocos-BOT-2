...
class Ranking(commands.Cog):
    ...
    async def perfil(self, ctx, jogador: discord.Member = None):
        ...
        Uso: .perfil  (Vê o seu)
        Uso: .perfil @Marocos (Vê o do amigo)
        """
        # Avisa que está processando (feedback visual importante em comandos de texto)
        async with ctx.typing():
            ...
            # --- DESIGN DO CARD (Mesmo do anterior) ---
            ...
            if player.riot_icon_id:
                embed.set_thumbnail(url=f"http://ddragon.leagueoflegends.com/cdn/14.1.1/img/profileicon/{player.riot_icon_id}.png")
